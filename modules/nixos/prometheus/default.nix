...
in
{
  ...
  config = mkIf cfg.enable {
    services.prometheus = {
        ...
        (
          ...
          builtins.filter
            (
              v:
              let
                static_configs = builtins.head v.static_configs;
              in
              ...
              mapAttrsToList
                (name: value: {
                  ...
                  static_configs =
                    let
                      hosts = filterAttrs (
                        host: nixosConfig: nixosConfig.config.services.prometheus.exporters.${name}.enable or false
                      ) outputs.nixosConfigurations;
                    in
                    [
                      {
                        targets = lib.mapAttrsToList (
                          host: nixosConfig: "${host}.host.bddvlpr.cloud:${toString value.port}"
                        ) hosts;
                      }
                    ];
                })
                (
                  filterAttrs (
                    name: value: !(builtins.elem name ignoredExporters)
      ...
      alertmanagers =
        let
          alertmanagerConfigs = filterAttrs (
            ...
            static_configs = [
              {
                targets = mapAttrsToList (
                  ...
                  in
                  "${host}.host.bddvlpr.cloud:${toString port}"
                ) alertmanagerConfigs;
              }
            ];
          }
